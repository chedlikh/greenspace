...
import { useCreateProgramme } from '../../../../services/formation';
import { Briefcase, Save, X, Loader2, AlertCircle } from "lucide-react";

// Theme color mapping based on CreatePoste
const themeColors = {
...
// InputField component (adapted from CreatePoste)
...
const ProgrammeCreate = () => {
  ...
  useEffect(() => {
    console.log("Token in ProgrammeCreate:", token);
  }, [token]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await createProgramme.mutateAsync(formData, {
        onSuccess: (data) => {
        ...
        <div className="bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-200">
          ...
          <div className="bg-gradient-to-r from-[color:var(--theme-primary)] to-[color:var(--theme-secondary)] px-8 py-8">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h1 className="text-3xl font-bold text-white">Créer un nouveau programme</h1>
                <p className="text-white/80 mt-1">Remplissez les informations ci-dessous pour créer un programme</p>
              </div>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} noValidate className="p-8 space-y-10">
            ...
            {/* Error Alert */}
            {(formError || createProgramme.isError || !token) && (
              <div className="border-l-4 border-red-500 p-4 rounded-lg bg-red-50 animate-fadeIn">
                <div className="flex items-center space-x-2">
                  ...
                  <p className="text-red-700">
                    {formError || (!token ? "Authentification requise. Veuillez vous connecter." : createProgramme.error?.message || "Échec de la création du programme")}
                  </p>
                </div>
              </div>
            )}
            ...
            <div className="flex flex-col sm:flex-row justify-end space-y-3 sm:space-y-0 sm:space-x-4 pt-8 border-t border-gray-200 animate-fadeIn">
              ...
              <button
                type="submit"
                disabled={createProgramme.isLoading || Object.values(errors).some((e) => e)}
              ...
              >
                {createProgramme.isLoading && <LoadingSpinner />}
                <Save size={18} />
